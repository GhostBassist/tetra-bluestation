...
impl DTxInterrupt {
    ...
    pub fn from_bitbuf(buffer: &mut BitBuffer) -> Result<Self, PduParseError> {
        ...
        let transmitting_party_address_ssi = if obit { 
            typed_pdu_fields::type2::parse(buffer, 24, "transmitting_party_address_ssi")? as Option<u64>
        } else { None };
        // Type2
        let transmitting_party_extension = if obit { 
            typed_pdu_fields::type2::parse(buffer, 24, "transmitting_party_extension")? as Option<u64>
        } else { None };
    ...
    pub fn to_bitbuf(&self, buffer: &mut BitBuffer) -> Result<(), PduParseError> {
        ...
        let obit_val = self.notification_indicator.is_some() || self.transmitting_party_type_identifier.is_some() || self.transmitting_party_address_ssi.is_some() || self.transmitting_party_extension.is_some() || self.external_subscriber_number.is_some() || self.facility.is_some() || self.dm_ms_address.is_some() || self.proprietary.is_some() ;
        ...
        // Type2
        typed_pdu_fields::type2::write(buffer, self.transmitting_party_address_ssi, 24);

        // Type2
        typed_pdu_fields::type2::write(buffer, self.transmitting_party_extension, 24);
