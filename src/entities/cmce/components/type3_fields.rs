use crate::{common::{bitbuffer::BitBuffer, pdu_parse_error::PduParseError, typed_pdu_fields::delimiters}, entities::cmce::enums::type3_elem_id::CmceType3ElemId};

#[derive(Debug, PartialEq, Eq)]
pub struct CmceType3Field {
...
impl CmceType3Field {
    ...
    pub fn parse(buffer: &mut BitBuffer, field_name: &'static str) -> Result<Option<Self>, PduParseError> {
        let m = delimiters::read_mbit(buffer)?;
        if m {
            let id = buffer.read_field(4, field_name)?;
            let id = CmceType3ElemId::try_from(id).map_err(|_| PduParseError::InvalidType3ElemId { found: id })?;
            let len_bits = buffer.read_field(11, field_name)? as usize;
            let data = buffer.read_field(len_bits, field_name)?;
            Ok(Some(CmceType3Field { field_type: id, len: len_bits, data }))
        } else {
            Ok(None)
        }
    }
