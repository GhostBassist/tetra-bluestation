...
impl DNwrkBroadcast {
    ...
    pub fn from_bitbuf(buffer: &mut BitBuffer) -> Result<Self, PduParseError> {
        ...
        // Type2
        let tetra_network_time = if obit { 
            typed_pdu_fields::type2::parse(buffer, 48, "tetra_network_time")? as Option<u64>
        } else { None };
        // Type2
        let number_of_ca_neighbour_cells = if obit { 
            typed_pdu_fields::type2::parse(buffer, 3, "number_of_ca_neighbour_cells")? as Option<u64>
        } else { None };
        // Conditional
    ...
    pub fn to_bitbuf(&self, buffer: &mut BitBuffer) -> Result<(), PduParseError> {
        ...
        let obit_val = self.tetra_network_time.is_some() || self.number_of_ca_neighbour_cells.is_some() ;
        typed_pdu_fields::delimiters::write_obit(buffer, obit_val as u8);
        if !obit_val { return Ok(()); }

        // Type2
        typed_pdu_fields::type2::write(buffer, self.tetra_network_time, 48);

        // Type2
        typed_pdu_fields::type2::write(buffer, self.number_of_ca_neighbour_cells, 3);
