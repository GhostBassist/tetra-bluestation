...
impl UPrepare {
    ...
    pub fn from_bitbuf(buffer: &mut BitBuffer) -> Result<Self, PduParseError> {
        ...
        // Type2
        let cell_identifier_ca = if obit { 
            typed_pdu_fields::type2::parse(buffer, 5, "cell_identifier_ca")? as Option<u64>
        } else { None };
        // Conditional
    ...
    pub fn to_bitbuf(&self, buffer: &mut BitBuffer) -> Result<(), PduParseError> {
        ...
        let obit_val = self.cell_identifier_ca.is_some() ;
        typed_pdu_fields::delimiters::write_obit(buffer, obit_val as u8);
        if !obit_val { return Ok(()); }

        // Type2
        typed_pdu_fields::type2::write(buffer, self.cell_identifier_ca, 5);
