...
impl MmType3FieldDl {
    pub fn parse(buffer: &mut BitBuffer, expected_id: MmType34ElemIdDl) -> Result<Self, Type34Err> { 
        let (len, data) = parse_type3_generic(buffer, expected_id.into_raw())?;
        Ok(MmType3FieldDl { field_type: expected_id, len, data })
    }
