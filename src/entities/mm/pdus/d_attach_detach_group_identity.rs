...
pub struct DAttachDetachGroupIdentity {
    ...
    /// Type3, See note,
    pub group_report_response: Option<MmType3FieldDl>,
    /// Type4, See note,
    pub group_identity_downlink: Option<MmType4FieldDl>,
...
impl DAttachDetachGroupIdentity {
    ...
    pub fn from_bitbuf(buffer: &mut BitBuffer) -> Result<Self, PduParseError> {
        ...
        // Type3
        let group_report_response = match MmType3FieldDl::parse(buffer, MmType34ElemIdDl::GroupReportResponse) {
            Ok(value) => Some(value),
            Err(_) => {None}
        };
        
        // Type4
        let group_identity_downlink = match MmType4FieldDl::parse(buffer, MmType34ElemIdDl::GroupIdentityDownlink) {
            Ok(value) => Some(value),
            Err(_) => {None}
        };
    ...
    pub fn to_bitbuf(&self, buffer: &mut BitBuffer) -> Result<(), PduParseError> {
        ...
        let obit_val = self.proprietary.is_some() || self.group_report_response.is_some() || self.group_identity_downlink.is_some() || self.group_identity_security_related_information.is_some() ;
        ...
        // Type4
        if let Some(ref value) = self.group_identity_downlink {
