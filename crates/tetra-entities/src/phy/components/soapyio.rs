        ...
        if let Some(rx) = &mut self.rx {
            ...
            match rx.read(&mut [buffer], 1000000) {
                Ok(len) => {
                    ...
                    let mut initial_time = self.initial_time.unwrap();
                    let mut count = time_ns_to_ticks(time - initial_time, self.rx_fs);
                    ...
                    if delta_from_expected.abs() <= RX_TIMESTAMP_JITTER_TOLERANCE_SAMPLES {
                        if delta_from_expected != 0 {
                            // Re-anchor phase so persistent +/-1 sample offset does not repeat forever.
                            initial_time += ticks_to_time_ns(delta_from_expected, self.rx_fs);
